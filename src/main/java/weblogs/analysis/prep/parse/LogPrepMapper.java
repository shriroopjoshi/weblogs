package weblogs.analysis.prep.parse;
...
import weblogs.analysis.prep.util.ParamUtil;

public class LogPrepMapper extends Mapper<LongWritable, Text, NullWritable, Text> {
	// 50.57.190.149 - - [22/Apr/2012:07:12:42 +0530] "GET /a/b/c/d?p=10
	// HTTP/1.0" 200 12530 "-" "-"
	...
	public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
		...
		String formattedValue = value.toString().replaceAll("\t", " ").trim(); // remove
																				// \t
																				// as
																				// it
																				// is
		...
		if (matcher.matches() && NUM_FIELDS == matcher.groupCount()) {
			String requestString = matcher.group(5); // Request String = Method
														// request protocol
			if (requestString != null) {
				String separateReqCategory = getSeparateReqCategories(requestString);

				StringBuffer valueBuffer = new StringBuffer();
				valueBuffer.append(matcher.group(1)).append(ParamUtil.DELIMITER_TAB); // remoteIP
																						// 14.97.118.184
				...
				valueBuffer.append(separateReqCategory).append(ParamUtil.DELIMITER_TAB); // cat1
																							// cat2
																							// cat3
																							// cat4
																							// page
				...
				valueBuffer.append(matcher.group(9));// .append(ParamUtil.DELIMITER_TAB);
														// //browser
	...
	String getProcessedRequest(String request) {
		...
		String requestParamTokens[] = request.split("\\?"); // reqParam =
															// /a/b?aaa
		...
		if (paramFlag)
			requestTokens = requestParamTokens[0].split("/"); // Request =
																// /a/b/c
		else
